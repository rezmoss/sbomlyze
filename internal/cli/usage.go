...
func PrintUsage() {
	...
	fmt.Fprintf(os.Stderr, "Use - in place of one SBOM path to read it from standard input.\n\n")
	...
	fmt.Fprintf(os.Stderr, "  syft . -o cyclonedx-json | sbomlyze -      # Analyze from standard input\n")
	fmt.Fprintf(os.Stderr, "  syft . -o cyclonedx-json | sbomlyze old -  # Diff stdin against a file\n")
