...
func TestDetectDuplicates(t *testing.T) {
	...
	t.Run("empty input returns no duplicates", func(t *testing.T) {
		dups := detectDuplicates([]Component{})
		if len(dups) != 0 {
			t.Errorf("expected 0 duplicate groups, got %d", len(dups))
		}
	})
...
func TestDiffComponentsWithDuplicates(t *testing.T) {
	t.Run("reports duplicates in before", func(t *testing.T) {
		before := []Component{
			{ID: "pkg:npm/lodash", Name: "lodash", Version: "4.17.20"},
			{ID: "pkg:npm/lodash", Name: "lodash", Version: "4.17.21"},
		}
		...
		if result.Duplicates == nil {
			t.Fatal("expected duplicates report, got nil")
		}
		if len(result.Duplicates.Before) != 1 {
			t.Errorf("expected 1 duplicate in before, got %d", len(result.Duplicates.Before))
		}
		if len(result.Duplicates.After) != 0 {
			t.Errorf("expected 0 duplicates in after, got %d", len(result.Duplicates.After))
		}
	})

	t.Run("reports duplicates in after", func(t *testing.T) {
		before := []Component{
			{ID: "pkg:npm/lodash", Name: "lodash", Version: "4.17.21"},
		}
		after := []Component{
		...
		if len(result.Duplicates.Before) != 0 {
			t.Errorf("expected 0 duplicates in before, got %d", len(result.Duplicates.Before))
		}
		if len(result.Duplicates.After) != 1 {
			t.Errorf("expected 1 duplicate in after, got %d", len(result.Duplicates.After))
		}
	})

	t.Run("no duplicates report when none exist", func(t *testing.T) {
		before := []Component{
			{ID: "pkg:npm/lodash", Name: "lodash", Version: "4.17.20"},
		}
		after := []Component{
		...
		if result.Duplicates != nil {
			t.Errorf("expected no duplicates report, got %+v", result.Duplicates)
		}
	})
}
